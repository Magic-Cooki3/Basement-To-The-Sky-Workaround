                    ...
                    if (method != null && method.HasBody) {
                        ...
                        for (int i = 0; i < insts.Count; i++) {
                            if (insts[i].OpCode == OpCodes.Ldstr && (string)insts[i].Operand == "Score") {
                                // Assume sequence: ldstr "Score", ldarg.0, ldfld score, call ES3.Save
                                var ldstrInst = insts[i];
                                var ldarg0Inst = insts[i+1];
                                var ldfldInst = insts[i+2];
                                var callInst = insts[i+3];
                                
                                if (callInst.OpCode == OpCodes.Call && callInst.Operand is MethodReference mr && mr.Name == "Save") {
                                    processor.Remove(ldstrInst); // remove ldstr
                                    processor.Replace(callInst, processor.Create(OpCodes.Call, addScoreRef)); // replace call
                                    fixes++;
